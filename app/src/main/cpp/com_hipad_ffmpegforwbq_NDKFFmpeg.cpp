#include "com_hipad_ffmpegforwbq_NDKFFmpeg.h"
#include <android/log.h>


#define LOGI(FORMAT,...) __android_log_print(ANDROID_LOG_INFO,"wbq",FORMAT,__VA_ARGS__);
#define LOGE(FORMAT,...) __android_log_print(ANDROID_LOG_ERROR,"wbq",FORMAT,__VA_ARGS__);

//当前C++兼容C语言
extern "C"{
//avcodec:编解码(最重要的库)
#include "libavcodec/avcodec.h"
//avformat:封装格式处理
#include "libavformat/avformat.h"
//avutil:工具库(大部分库都需要这个库的支持)
#include "libavutil/avutil.h"
//swscale:视频像素数据格式转换
#include "libswscale/swscale.h"
}
JNIEXPORT void JNICALL
Java_com_hipad_ffmpegforwbq_NDKFFmpeg_callFFmpegDecode(JNIEnv *env, jobject jobj,
                                                          jstring input_jstr,
                                                          jstring output_jstr){
    //java sting转成c中的string
    const char* input_cstr=env->GetStringUTFChars(input_jstr,NULL);
    const char* output_cstr=env->GetStringUTFChars(output_jstr,NULL);
    //1，注册组件
    av_register_all();
    //封装格式上下文结构体，保存了视频文件的封装格式相关信息
    AVFormatContext *avFmtCtx=avformat_alloc_context();
//    avFmtCtx->iformat duration (时常等)
    //2,打开输入视频文件
    int format_open_result=avformat_open_input(&avFmtCtx,input_cstr,NULL,NULL);
    if(format_open_result!=0){
        LOGE("%s","打开输入视频文件失败或者文件不存在");
        return;
    }
    //3,获取视频文件 参数2 avDictionary 获取视频元数据 key value格式
    //很多流（例如：视频流、音频流、字幕流等等......）
    //然后我的目的：我只需要视频流信息
    int fmt_fd_into=avformat_find_stream_info(avFmtCtx,NULL);
    if(fmt_fd_into<0){
        LOGE("%s","获取视频信息失败");
        //打印错误码
        //错误信息
        char* error_info;
        //根据错误码找到对应的错误信息s
        av_strerror(fmt_fd_into,error_info,1024);
        LOGE("错误信息：%s",error_info);
        return;
    }
    //4：查找解码器
    //1.1获取视频流的索引位置
    //遍历所有的流，找到视频流
    int video_stream_index = 0;
    int i=0;
    for(;i<avFmtCtx->nb_streams;i++){
        //根据类型判断，是否是视频流
        if(avFmtCtx->streams[i]->codec->codec_type==AVMEDIA_TYPE_VIDEO){
            video_stream_index=i;
            break;
        }
    }
    if (video_stream_index==-1){
        LOGI("%s","不存在视频流");
        return;
    }
    //1.2根据视频流的索引位置，查找视频流的解码器
    //根据视频流的索引位置，获取到指定的参数上下文
    //编码方式(编码上下文)
    AVCodecContext *avCodecCtx=avFmtCtx->streams[video_stream_index]->codec;
    //根据codec_id拿到解码器
    AVCodec *avCodec=avcodec_find_decoder(avCodecCtx->codec_id);
    if (avCodec==NULL){
        LOGE("%s","视频进行了加密或其他处理无法解码");
        return;
    }
    //5，打开解码器
    int av_cd_open_result=avcodec_open2(avCodecCtx,avCodec,NULL);
    if (av_cd_open_result!=0){
        LOGE("%s","打开解码器失败");
        return;
    }
    //获取配置视频信息
    //文件格式、文件的宽高、解码器的名称等等......
    LOGE("视频文件的格式：%s",avFmtCtx->iformat->name);
    //返回的单位是：微秒(avFmtCtx->duration)
    LOGE("视频的时长：%lld秒",(avFmtCtx->duration)/1000000);
    //获取宽高
    LOGE("视频的宽高：%d x %d = ",avCodecCtx->width,avCodecCtx->height);
    //视频解码器的名称
    LOGE("解码器的名称：%s",avCodec->name);

    //6，从输入文件读取一帧压缩数据(解压缩:一帧一帧读取解压缩)
    //循环读取每一帧数
    //读取的帧数据缓存到那里(开辟一块内存空间用于保存)
//    AVPacket *avPacket= (AVPacket *) malloc(sizeof(AVPacket*));
    AVPacket* packet = (AVPacket*)av_malloc(sizeof(AVPacket));
    //缓存一帧数据(就是一张图片)
    AVFrame* in_frame_picture = av_frame_alloc();
//定义输出一帧数据(缓冲区:YUV420p类型)
    AVFrame* out_frame_picture_YUV42P = av_frame_alloc();
    //指定缓冲区类型 像素格式：YUV420P
    //开启空间大小YUV420P数据格式大小
    uint8_t *out_buffer= (uint8_t *) av_malloc(avpicture_get_size(AV_PIX_FMT_YUV420P, avCodecCtx->width, avCodecCtx->height));
    //指定填充数据类型，此时才会真正的开辟内存空间
    avpicture_fill((AVPicture *) out_frame_picture_YUV42P, out_buffer, AV_PIX_FMT_YUV420P, avCodecCtx->width, avCodecCtx->height);


    FILE* out_file_yuv=fopen(output_cstr,"rw");
    if (out_file_yuv==NULL){
        LOGI("%s","文件不存在!");
        return;
    }
    //视频像素数据格式转换上下文 也可以对视频进行缩放
    //参数一：输入的宽度
    //参数二：输入的高度
    //参数三：输入的数据
    //参数四：输出的宽度
    //参数五：输出的高度
    //参数六：输出的数据
    //参数七：视频像素数据格式转换算法类型(使用什么算法)
    //参数八：字节对齐类型，一般都是默认1（字节对齐类型：提高读取效率）
    SwsContext *sws_ctx =
            sws_getContext(avCodecCtx->width,avCodecCtx->height, avCodecCtx->pix_fmt,
                           avCodecCtx->width,avCodecCtx->height,AV_PIX_FMT_YUV420P,
                           SWS_BICUBIC,NULL,NULL,NULL);

    int len=0;
    int got_frame=0;//解码的返回值 用来确定解码成功或者失败
    int frame_index=0;
    //读取返回值
    //>=0：正在读取
    //<0:读取失败或者说读取完毕
    while(av_read_frame(avFmtCtx,packet)>=0){
        //有视频流帧数据、音频流帧数据、字幕流......
        if(packet->stream_index == video_stream_index){
            len= avcodec_decode_video2(avCodecCtx,in_frame_picture,&got_frame,packet);
            if (len<0){
                LOGI("%s","解码失败");
                return;
            }
            //get_frame 0 解码完成 非0 正在解码
            if (got_frame){
                //接下来我要将解码后的数据（视频像素数据，保存为YUV420P文件）
                //在这个地方需要指定输出文件的类型(格式转换)
                //我要将AVFrame转成视频像素数YUV420P格式
                //参数一：视频像素数据格式上下文(SwsContext)
                //参数二：输入的数据（转格式前的视频像素数据）
                //参数三：输入画面每一行的大小(视频像素数据转换一行一行的转)
                //参数四：输入画面每一行的要转码的开始位置
                //参数五：输出画面数据(转格式后的视频像素数据)
                //参数六：输出画面每一行的大小
                sws_scale(sws_ctx, (const uint8_t *const *) in_frame_picture->data, in_frame_picture->linesize, 0, avCodecCtx->height, out_frame_picture_YUV42P->data, out_frame_picture_YUV42P->linesize);

                //普及: YUV420P格式结构
                //Y代表亮度，UV代表色度（人的眼睛对亮度敏感，对色度不敏感）
                //再深入：计算机图像学相关
                //YUV420P格式规定一：Y结构表示一个像素点(一个像素点就是一个Y)
                //YUV420P格式规定二：四个Y对应一个U和一个V(也就是四个像素点，对应一个U和V)
                //Y默认情况下：灰度
                //计算Y大小：y = 宽x高
                int y_size = avCodecCtx->width * avCodecCtx->height;
                int u_size = y_size / 4;
                int v_size = y_size / 4;
                //写入文件
                //首先写入Y，再是U，再是V
                //in_frame_picture->data[0]表示Y
                fwrite(in_frame_picture->data[0],1,y_size,out_file_yuv);
                //in_frame_picture->data[1]表示U
                fwrite(in_frame_picture->data[1],1,u_size,out_file_yuv);
                //in_frame_picture->data[2]表示V
                fwrite(in_frame_picture->data[2],1,v_size,out_file_yuv);

                frame_index++;
                LOGI("当前解码了是第%d帧",frame_index);

            }
        }
        av_free_packet(packet);
    }
    //关闭流
    fclose(out_file_yuv);
    av_frame_free(&in_frame_picture);
    av_frame_free(&out_frame_picture_YUV42P);
    avcodec_close(avCodecCtx);
    avformat_free_context(avFmtCtx);
    //释放
    env->ReleaseStringUTFChars(input_jstr,input_cstr);
    env->ReleaseStringUTFChars(output_jstr,output_cstr);

}




